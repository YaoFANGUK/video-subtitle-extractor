...
class FFmpeg(object):
    ...
    def get_info(path):
        try:
            ...
            process = subprocess.Popen([ffmpeg_path, '-hide_banner', '-i', path], stderr=subprocess.PIPE, universal_newlines=True)
    ...
    def demux_file(input_path, **kwargs):
        args = [ffmpeg_path, '-hide_banner', '-i', input_path, '-y']
...
class SCXviD(object):
    ...
    def make_keyframes(cls, video_path, log_path):
        try:
            ffmpeg_process = subprocess.Popen([ffmpeg_path, '-i', video_path,
                                               '-f', 'yuv4mpegpipe',
                                               '-vf', 'scale=640:360',
                                               '-pix_fmt', 'yuv420p',
                                               '-vsync', 'drop', '-'],
